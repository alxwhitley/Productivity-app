...
// ── Save: local for now, Supabase-ready ─────────────────────────────────────
async function saveData(data) {
  if (USE_SUPABASE && SUPABASE_URL) {
    // Future: await supabase.from("user_data").upsert({ user_id, data })
    return;
  }
  try {
...
// ─── TODAY SCREEN ─────────────────────────────────────────────────────────────
function TodayScreen({ data, setData, openShutdown, openAddBlock, focusMode: focusModeprop, setFocusMode: setFocusModeApp }) {
    ...
    <>
      ...
      <div style={{ height:"100%", display:"flex", alignItems:"center", justifyContent:"center", background: lightMode ? "#E8E4DE" : "#101213" }}>
        <div className={`phone ${lightMode ? "light" : ""}`}>
          {tab==="today"    && <TodayScreen    data={data} setData={setData} openShutdown={()=>setSheet("shutdown")} openAddBlock={()=>setSheet("addblock")} focusMode={focusMode} setFocusMode={setFocusMode} />}
