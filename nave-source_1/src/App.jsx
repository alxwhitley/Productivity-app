...
function getRoutineDateKey(date) {
          ...
          <div className="tl-wrap" style={{ paddingTop: 4 }}>
            {timeline.map((item, idx) => {
              ...
              if (item.type === "block") {
                ...
                const showFullCheck = isCompleted && (relevantTasks.length === 0 || allTasksDone);
                const showPartialCheck = isCompleted && relevantTasks.length > 0 && !allTasksDone;
                ...
                return (
                  <div key={item.id} className="tl-item">
                    ...
                    <div className={cardClass} style={{ "--domain-color": domainCardColor + "50" }}>
                      {/* Conflict warning */}
                      {showConflict && (
...
function ProjectsScreen({ data, setData, openCategorize }) {
  ...
  const saveProjectName = (projectId) => {
    ...
    if (name) setData(d => ({ ...d, projects: d.projects.map(p => p.id === projectId ? { ...p, name } : p) }));
...
function LoginScreen() {
  ...
  return (
    <div style={{ height:"100%", display:"flex", alignItems:"center", justifyContent:"center", background:"#101213" }}>
      <div style={{ width:340, padding:"40px 32px", background:"#1E2122", borderRadius:24, border:"1px solid #252829" }}>
        <div style={{ fontSize:28, fontWeight:700, color:"#fff", marginBottom:4, fontFamily:"'DM Sans',sans-serif" }}>Nave</div>
        <div style={{ fontSize:13, color:"#666", marginBottom:32, fontFamily:"'DM Sans',sans-serif" }}>Your deep work command center</div>
        {!sent ? (
          <>
            <div style={{ fontSize:13, color:"#aaa", marginBottom:8, fontFamily:"'DM Sans',sans-serif" }}>Email</div>
            <input
              type="email"
              value={email}
              ...
              style={{ width:"100%", padding:"11px 14px", background:"#252829", border:"1px solid #2C3032", borderRadius:10, color:"#fff", fontSize:14, fontFamily:"'DM Sans',sans-serif", outline:"none", marginBottom:12 }}
              autoFocus
            />
            ...
            <button
              ...
              style={{ width:"100%", padding:"12px", background: email.trim() ? "#E8A030" : "#333", color: email.trim() ? "#000" : "#666", border:"none", borderRadius:10, fontSize:14, fontWeight:700, cursor: email.trim() ? "pointer" : "default", fontFamily:"'DM Sans',sans-serif", transition:"all .15s" }}
            >
          ...
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontSize:15, fontWeight:600, color:"#fff", marginBottom:8, fontFamily:"'DM Sans',sans-serif" }}>Check your email</div>
            <div style={{ fontSize:13, color:"#888", fontFamily:"'DM Sans',sans-serif", lineHeight:1.5 }}>We sent a magic link to <strong style={{color:"#ccc"}}>{email}</strong>. Click it to sign in.</div>
            <button onClick={() => setSent(false)} style={{ marginTop:20, background:"none", border:"none", color:"#E8A030", fontSize:12, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>Use a different email</button>
          </div>
        )}
      </div>
    </div>
